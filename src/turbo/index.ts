...
export default async function TurboDeploy(deployFolder: string, jwk) {
  const turbo = TurboFactory.authenticated({ privateKey: jwk });

  const defaultFile = fs.existsSync(path.join(deployFolder, "index.html"))
    ? "index.html"
    : "versions.json";
        ...
        if (fs.statSync(filePath).isDirectory()) {
          ...
          try {
            ...
            const uploadResult = await turbo.uploadFile({
              ...
              dataItemOpts: {
                tags: [
                  { name: "Content-Type", value: contentType as string},
                  { name: "App-Name", value: "Permaversion" },
                ],
              },
            });
  ...
  if (!newManifest.fallback.id) {
    ...
    newManifest.fallback.id = newManifest.paths[defaultFile].id;
  }
